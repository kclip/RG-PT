...
#                                                       calibration_error)
#
#         for i, (prob, true) in enumerate(zip(probs, y_test)): # Recall
...
#             # Recall is calculated as recall_score(y_true, y_pred, average='binary' or 'macro')
...
data = np.load("processed_results_recall.npz")
accuracy_errors = data["accuracy_errors"]
calibration_errors = data["calibration_errors"]
...
lr_toplot, adjacency_matrix, efficient_ids = Pareto_testing(accuracy_errors, calibration_errors,0.35, 0.1, num_calib, length)
...
gamma_values = np.log(gamma_values)
lambdas = itertools.product(C_values, gamma_values)
...
pos = {i: [lambdas_chosen[i, 0], lambdas_chosen[i, 1], z[efficient_ids[i]]] for i in G.nodes}
...
contour = ax.contourf(X, Y, Z, cmap='coolwarm', levels=20)  # Contour plot with color filling

# Add color bar to indicate z values
...
# Extract the positions of the efficient nodes in 2D
lambdas_chosen = lambdas[efficient_ids, :]
pos = {i: [lambdas_chosen[i, 0], lambdas_chosen[i, 1]] for i in G.nodes}
...
for edge in G.edges:
    ...
    ax.annotate("",
                ...
                arrowprops=dict(arrowstyle="->", lw=1, color="black", linestyle = "dashed"))

# Plot nodes in 2D space with single colors for efficient and inefficient nodes
for node, (x_node, y_node) in pos.items():
    if node in lr_toplot:
        # Efficient nodes: use single green color
        color = "green"
        ax.scatter(x_node, y_node, c=color, s=100, label='Efficient' if not any(n in lr_toplot for n in pos) else "")
    else:
        # Inefficient nodes: use single red color
        color = "red"
        ax.scatter(x_node, y_node, c=color, s=100, label='Inefficient' if any(n not in lr_toplot for n in pos) else "")
...
padding = 0.1  # Set a padding ratio (10%)
...
plt.savefig("DAG_2D.pdf", format="pdf")
plt.show()
...
# Plot sequential
# Extract x, y, and z
lr_toplot, adjacency_matrix, efficient_ids = sequential_testing(accuracy_errors, calibration_errors,0.3, 0.1, num_calib, length)
...
# Save and show the plot
plt.savefig("Pareto_2D.pdf", format="pdf")
plt.show()
...
# Plot LTT
lr_toplot, adjacency_matrix, efficient_ids = ltt_testing(accuracy_errors, calibration_errors,0.28, 0.1, num_calib, length)
...
# Show the plot
ax.view_init(elev=30, azim=225)
plt.savefig("LTT.pdf", format="pdf")
plt.show()
...
# Save and show the plot
plt.savefig("LTT_2D.pdf", format="pdf")
plt.show()
