...
def Pareto_testing(calib1, calib2, alphas, delta):
    ...
    calib1 = calib1.T
    adjacency_matrix = create_DAG(calib1[efficient_ids, :], p_vals, alpha=np.min(alphas), method='regression')
...
def trial(i, method, alphas, delta, lambda1s, lambda2s, lambda3s, l1_meshgrid, l2_meshgrid, l3_meshgrid, num_calib, loss_tables, risks, lhats, num_par):
    ...
    if method == "Bonferroni":
        ...
        R = Pareto_testing(calib_tables1, calib_tables2, alphas, delta)
    elif method == "BH":
        calib_tables1, calib_tables2 = (calib_tables[:num_par], calib_tables[num_par:])
...
if __name__ == "__main__":
    ...
    df_list = []
    methods = ["Bonferroni", "Split Fixed Sequence", "DAG", "BH"]
        ...
        try:
            ...
            with torch.no_grad():
                ...
                for i in range(num_trials):
                    p = mp.Process(target = trial, args = (i, method, alphas, delta, lambda1s, lambda2s, lambda3s, l1_meshgrid, l2_meshgrid, l3_meshgrid, num_calib, loss_tables, risks, lhats, num_par))
                    jobs.append(p)
                ...
                df.to_pickle(fname)
                lhats_list = list(lhats.values())
                data_transposed = list(zip(*lhats_list))
                ...
                # Customize the box fill color
                for patch in box['boxes']:
                    patch.set(facecolor="orange")  # Light blue color with some transparency

                # Add grid lines for readability
                plt.xlabel('Values')
                #plt.title(method)
                plt.xlim([0.45,1])
                plt.savefig(f'{method}_{method}.pdf')
                plt.show()
