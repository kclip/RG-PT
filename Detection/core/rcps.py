...
from core.bounds_rcps import bentkus_mu_plus, HB_mu_plus, HBB_mu_plus, WSR_mu_plus
import torch
import math
...
def get_lhat_from_table_binarysearch(calib_loss_table, lambdas_table, gamma, delta, tlambda, bound_str):
    ...
    return [len(lambdas_table)-i-1] #TODO: correct?

def get_lhat_from_table_binarysearch_quantile(calib_loss_table, lambdas_table, gamma, delta, q):
    avg_loss = np.quantile(calib_loss_table, q, axis = 0)
    std_loss = calib_loss_table.std(axis=0)
    n = calib_loss_table.shape[0]
    u = 0.85*np.sqrt((np.log(np.log(math.e*n))+0.8*np.log(612/delta))/n)
    Q_hat_plus = np.quantile(calib_loss_table, q+u, axis=0)
    lb_idx = 1
    ub_idx = len(lambdas_table)-1
    i = 0
    ...
    while lb_idx != ub_idx-1:
        i = (lb_idx + ub_idx)//2
        Rhat = avg_loss[i]
        Rhat_plus = Q_hat_plus[i]
        if (Rhat > gamma) or (Rhat_plus > gamma):
            ub_idx = i
        else:
            lb_idx = i

    return [len(lambdas_table)-i-1]
