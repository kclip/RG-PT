...
def Quantile_UCB(loss_vec, q, delta):
    n = np.size(loss_vec, 0)
    u = 0.85*np.sqrt((np.log(np.log(math.e*n))+0.8*np.log(612/delta))/n)
    return np.quantile(loss_vec, min(q+u, 1))


def IB_UCB_YT(loss_vec, delta):
    theta = np.sqrt(2/n_test*np.log((2**(My*Mt)-2)/delta))
...
def Quantile_p_value(loss_vec, alpha, q, bisec_max_iter=20):
    # we first get the CI per delta
    def _no_intersect(loss_vec, delta):
        curr_UCB = Quantile_UCB(loss_vec, q, delta)  # delta -> u -> internal ..
...
def IB_p_value_YT(Y, T, alpha, bisec_max_iter=20):
    ...
    loss_vec = mutual_info_score(Y, T)
    delta = 1
    bi = 1
    for bisec_iter in range(bisec_max_iter):
        bi /= 2
...
def bonferroni_search_Quantile(loss_table,lambdas,alpha,delta,q,downsample_factor):
    n = loss_table.shape[0]
    num_lambdas = loss_table.shape[1]
    p_values = np.array([Quantile_p_value(loss_table[:, i][~np.isnan(loss_table[:, i])], alpha, q) for i in range(num_lambdas)])
    #print('p_values', p_values)
    #p_values = np.array([hb_p_value(r_hat,n,alpha) for r_hat in r_hats])
    return bonferroni_search(p_values,delta,downsample_factor)

def bonferroni_search_IB(loss_table,alpha,delta,downsample_factor):
    num_lambdas = len(loss_table)
    p_values = np.array([IB_p_value(loss_table[i][~np.isnan(loss_table[i])], alpha) for i in range(num_lambdas)])
    print(p_values)
    return bonferroni_search(p_values,delta,downsample_factor)

def IB_problem(X_cal, Y_cal, T_cal, alpha, delta):
    num_lambdas = X_cal.shape[1]
    p_value_XT = np.array([IB_p_value_XT(X_cal[:, j][~np.isnan(X_cal[:, j])], T_cal[:, j][~np.isnan(T_cal[:, j])], alpha[0]) for j in range(num_lambdas)])
    p_value_YT = np.array([IB_p_value_YT(Y_cal[:, j][~np.isnan(Y_cal[:, j])], T_cal[:, j][~np.isnan(T_cal[:, j])], alpha[1]) for j in range(num_lambdas)])
    return bonferroni_search_2D(p_value_XT, p_value_YT, delta)
...
def bonferroni_search_2D(p_values_1, p_values_2, delta, downsample_factor = 10):
    N = p_values_1.shape[0]
    N_coarse = max(int(p_values_1.shape[0] / downsample_factor), 1)
    # Downsample, making sure to include the endpoints.
    coarse_indexes = set(range(0, N, downsample_factor))
    coarse_indexes.update({0, N - 1})
    R = set()
    for idx in range(N):
        _idx = idx
        if p_values_1[_idx] < delta / (2*N) and p_values_2[_idx] < delta / (2*N):
            R.update({_idx})
    return np.array(list(R))
"""
